...
let posSrcLower = posSrc.toLowerCase()
let locSrcLower = locSrc.toLowerCase()
function search() {
  for (let i = 0; i < jobs.length; i++) {
    let location = jobs[i].location
    let locationLower = location.toLowerCase()
    let position = jobs[i].title
    let positionLower = position.toLowerCase()
    let job = jobs[i]
    if (locationLower.includes(locSrcLower) && positionLower.includes(posSrcLower)){
