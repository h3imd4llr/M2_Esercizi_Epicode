...
function checkArray() {
    for (let i = 0; i < giveMeRandom(array).length; i++) {
        if (array[i] > 5) {
            checkArr[i] = "true"
            upTo5[j] = array[i]
            j++
    ...
    const somma = upTo5.map(item => item).reduce((prev, curr) => prev + curr, 0);
...
const shoppingCart = {
        ...
        shipping_q: Math.floor(Math.random()*100)
    }

function shoppingCartTotal() {
    let CartTotal = shoppingCart.shipping_q * shoppingCart.price
    return {
        'Quantity': shoppingCart.shipping_q,
